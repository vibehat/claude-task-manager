...
import type { TaskManagerData } from '../../../../libs/client/services/taskManagerDataService';
...
export async function GET(): Promise<NextResponse> {
   ...
   try {
      ...
      // Merge with existing UI tasks (avoid duplicates by checking for tm- prefix)
      const existingTasks = finalData.tasks || [];
      const uiTasks = existingTasks.filter((task) => !task.id.startsWith('tm-'));
...
export async function POST(request: NextRequest): Promise<NextResponse> {
   try {
      ...
      console.log(`Saved ${uiTasks.length} UI tasks (excluded TaskMaster CLI tasks)`);

...
export const useMultiTerminalStore = create<MultiTerminalStore>()(
  persist(
    (set, get) => ({
      ...
      connectTerminal: async (id: string, options?: { sessionId?: string; clientId?: string }) => {
        const terminal = get().getTerminalById(id);
        ...
        try {
          ...
          return new Promise<void>((resolve, reject) => {
            ws.onopen = () => {
              ...
              set((state) => ({
                terminals: state.terminals.map((t) =>
                  t.id === id ? { ...t, connectionStatus: TerminalConnectionStatus.CONNECTED } : t
                ),
              }));
              resolve();
            };
      ...
      sendInput: (id: string, data: string) => {
        ...
        if (connectionData.websocket.readyState === WebSocket.OPEN) {
          ...
          console.warn(`Terminal ${id} WebSocket not open:`, connectionData.websocket.readyState);
        }
      },
      ...
      clearError: (id: string) => {
        set((state) => ({
          terminals: state.terminals.map((t) => (t.id === id ? { ...t, error: undefined } : t)),
        }));
      },
      ...
      handleTerminalMessage: (id: string, message: TerminalMessage) => {
        ...
        switch (message.type) {
          ...
          case 'error':
            ...
            set((state) => ({
              terminals: state.terminals.map((t) => (t.id === id ? { ...t, error: errorMsg } : t)),
            }));
            break;
      ...
      attemptReconnect: (id: string) => {
        ...
        set((state) => ({
          terminals: state.terminals.map((t) =>
            t.id === id ? { ...t, connectionStatus: TerminalConnectionStatus.RECONNECTING } : t
          ),
        }));
      ...
      cleanupStaleTerminals: () => {
        ...
        set((state) => ({
          terminals: state.terminals.filter((terminal) => {
            ...
            const timeSinceLastConnection = now.getTime() - terminal.lastConnectedAt.getTime();

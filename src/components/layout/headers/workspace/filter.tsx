...
import { usePriorities } from '@/libs/client/hooks/usePriorities';
import { useLabels } from '@/libs/client/hooks/useLabels';
import { useTags } from '@/libs/client/hooks/useTags';
import { getTaskStatusIcon } from '@/features/tasks/hooks/useTaskStatusIcon';
import { getTaskPriorityIcon } from '@/features/tasks/hooks/useTaskPriorityIcon';
import {
  CheckIcon,
  ChevronRight,
  ListFilter,
        ...
        {activeFilter === null ? (
          ...
          <Command>
            ...
            <CommandList>
              ...
              <CommandGroup>
                {statuses.map((item) => {
                  const StatusIcon = getTaskStatusIcon(item);
                  return (
                    <CommandItem
                      key={item.id}
                      value={item.id}
          ...
          <Command>
            ...
            <CommandList>
              ...
              <CommandGroup>
                {priorities.map((item) => {
                  const PriorityIcon = getTaskPriorityIcon(item);
                  return (
                    <CommandItem
                      key={item.id}
                      value={item.id}
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <PriorityIcon className="text-muted-foreground size-4" />
                        {item.name}
                      </div>
              ...
              <CommandGroup>
                {tags.map((tag) => (
                  ...
                  >
                    ...
                    {filters.tag.includes(tag.id) && <CheckIcon size={16} className="ml-auto" />}
                    <span className="text-muted-foreground text-xs">
                      {0 /* TODO: Now using local data*/}
                    </span>
                  </CommandItem>

...
export function SearchResultItem({
  ...
  const description =
    typeof command.description === 'function'
      ...
      : command.description || '';
  const icon =
    typeof command.icon === 'function' ? (context ? command.icon(context) : null) : command.icon;
      ...
      >
        ...
        {showMetadata && (
          <>
            ...
            <td className="px-3 py-2 text-xs font-mono text-muted-foreground">{command.id}</td>
          </>
        )}
      </tr>
    );

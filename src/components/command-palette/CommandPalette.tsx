    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="overflow-hidden p-0 !w-[650px] !max-w-[650px] max-h-[90vh] h-auto">
        <DialogHeader className="px-4 py-3 border-b border-border">
                  ...
                  {(() => {
                    ...
                    if (
                      ...
                      if (currentCommand.inputConfig.contextFormat) {
                        return currentCommand.inputConfig.contextFormat(title, inputValue);
                      }
          ...
          {currentCommand?.description && (
            <p className="text-sm text-muted-foreground mt-1">
              {typeof currentCommand.description === 'string' ? currentCommand.description : ''}
            </p>
          )}
        </DialogHeader>
        <div className="h-full flex flex-col" onKeyDown={handleKeyDown}>
          ...
          <CommandSearchInput
            mode={mode}
            value={mode === 'input' ? inputValue : mode === 'command-search' ? searchQuery : search}
            onValueChange={
              mode === 'input'
                ? setInputValue
                : mode === 'command-search'
          ...
          {/* Input mode hint */}
          {mode === 'input' && <CommandInputHint hint={inputConfig?.placeholder} />}

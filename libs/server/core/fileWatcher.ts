...
import { TaskMasterFileOperations } from './fsOperations';
...
export class EnhancedFileWatcher extends EventEmitter {
   ...
   private static globalInstanceCount = 0;
   ...
   constructor(config: FileWatcherConfig) {
      ...
      // Only add process listeners once globally
      if (EnhancedFileWatcher.globalInstanceCount === 0) {
         process.setMaxListeners(20); // Increase limit
         process.on('exit', () => this.close());
         process.on('SIGINT', () => this.close());
         process.on('SIGTERM', () => this.close());
      }
      EnhancedFileWatcher.globalInstanceCount++;
   }
   ...
   private handleFileChange(eventType: string, watchPath: string, filename: string): void {
      // Handle null/undefined filename (can happen with fs.watch)
      if (!filename) return;
   ...
   private async processFileChange(
      ...
      try {
         // Guard against undefined/null filepath
         if (!filepath) {
            console.warn('processFileChange called with undefined filepath');
            return;
         }
   ...
   close(): void {
      ...
      EnhancedFileWatcher.globalInstanceCount = Math.max(
         0,
         EnhancedFileWatcher.globalInstanceCount - 1
      );

...
describe('sync-features.js', () => {
  ...
  describe('getExistingFeaturesConfig', () => {
    ...
    it('should handle invalid JSON and return defaults', () => {
      ...
      expect(console.warn).toHaveBeenCalledWith('⚠️  Could not parse existing release.json, using defaults');
    });
  });
  ...
  describe('scanDomainsDirectories', () => {
    it('should scan and return domain directories', () => {
      ...
      expect(fs.readdirSync).toHaveBeenCalledWith(
        expect.stringContaining('apps/api/src/domains'),
        { withFileTypes: true }
      );
    });
    ...
    it('should handle multiple feature roots', () => {
      ...
      fs.existsSync.mockReturnValue(true);
      fs.readdirSync
        .mockReturnValueOnce(mockDirents1)
        .mockReturnValueOnce(mockDirents2);
  ...
  describe('mergeFeatures', () => {
    ...
    it('should log skipped and added features', () => {
      ...
      expect(console.log).toHaveBeenCalledWith('📊 Summary: 1 new features added, 1 existing features preserved');
    });
  ...
  describe('checkSyncStatus', () => {
    ...
    it('should return true and log error when an exception occurs', () => {
      ...
      expect(result).toBe(true);
      expect(console.error).toHaveBeenCalledWith('❌ Error checking sync status:', 'File system error');
    });
  ...
  describe('DEFAULT_FEATURES_ROOTS', () => {
    it('should export the default features roots constant', () => {
      expect(DEFAULT_FEATURES_ROOTS).toEqual([

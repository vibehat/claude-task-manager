...
class TaskCreator {
    ...
    try {
      ...
      if (error.code === 'ENOENT') {
        ...
        if (templateName !== 'default') {
          console.warn(`Template "${templateName}" not found, falling back to default template`);
  ...
  async showHelp() {
    ...
    const templateList = availableTemplates.map(name => {
...
`);
  ...
  async fetchJiraData(jiraUrl, panelTitle = null) {
    ...
    if (panelTitle) {
      ...
      if (!panel) {
        throw new Error(`Panel "${panelTitle}" not found. Available panels: ${Object.keys(ticketData.panels || {})}`);
      }
      return {
  ...
  async generateTaskFile(config, taskData) {
    ...
    if (taskData.jiraUrl) {
      ...
      content = content.replace('## Instructions for Claude', `${sourceSection}## Instructions for Claude`);
    }

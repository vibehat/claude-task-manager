...
const { mergeFeatures } = require('./sync-features');
...
totalTests++;
if (
   test('mergeFeatures should use raw folder names (should now PASS)', () => {
...
totalTests++;
if (
   test('mergeFeatures with complex domain names (should now use raw names)', () => {
      const existingFeatures = {};
...
totalTests++;
if (
   test('mergeFeatures should handle multiple feature roots with raw names', () => {
      const existingFeatures = {};
